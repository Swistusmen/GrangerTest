...
currencyConversion<- function(df, currencyFile, tableIndex){
  ...
  currency <- read.csv(currencyFile,sep=",")
...
sp500 <- read.csv("sp500.csv", sep=",") #USD
uk100 <- read.csv("uk100.csv",sep=",") # GBP
dax <- read.csv("dax.csv", sep=",") # EUR
nikkei <- read.csv("nikkei.csv",sep=",") #JPY
wig20 <-read.csv("wig20.csv",sep=",") #PLN
...
#usuniecie logarytmicznych 
colnames(logReturnCost)<- c("logReturnSP500","logReturnUK100","logReturnDax","logReturnNikkei","logReturnWig20")
VARselect(logReturnCost,lag.max=7)

#model VAR
model<- VAR(logReturnCost,p=1, type="none")
summary(model)

#autokorelacja
acf(model$varresult$logReturnSP500$residuals)
acf(model$varresult$logReturnUK100$residuals)
acf(model$varresult$logReturnDax$residuals)
acf(model$varresult$logReturnNikkei$residuals)
acf(model$varresult$logReturnWig20$residuals)

#test Grangera
grangerSP500<- causality(model, cause="logReturnSP500")
